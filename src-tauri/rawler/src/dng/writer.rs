...
use crate::{
    ...
    formats::tiff::{
        CompressionMethod, PhotometricInterpretation, PreviewColorSpace, Rational, TiffError,
        Value,
...
use super::{
    CropMode, DNG_VERSION_V1_6, DngCompression, DngPhotometricConversion,
    original::OriginalCompressed,
};
    ...
    B: Write + Seek,
{
    pub fn new(writer: &'w mut DngWriter<B>, subtype: u32, use_root: bool) -> Self {
        let ifd = if use_root {
            writer
                .root_ifd_mut()
                .add_tag(TiffCommonTag::NewSubFileType, subtype);
            None
        } else {
    ...
    pub fn rgb_image_u8(
        ...
        let cpp = 3;
        let rawimagedata = PixU16::new_with(
    ...
    fn write_rawimage(
        &mut self,
        mut rawimage: Cow<RawImage>,
        cropmode: CropMode,
        compression: DngCompression,
        predictor: u8,
    ) -> Result<()> {
        if compression == DngCompression::Lossless
            && matches!(rawimage.data, RawImageData::Float(_))
        {
        ...
        {
            self.writer
                .as_shot_neutral(wbcoeff_to_tiff_value(&rawimage));
            ...
            if let Some(first_key) = available_matrices.keys().next().cloned() {
                ...
                self.writer.color_matrix(
                    1,
                    first_matrix.0,
                    matrix_to_tiff_value(&first_matrix.1, 10_000).as_slice(),
                );
                    ...
                    .or_else(|| available_matrices.remove_entry(&Illuminant::D50))
                {
                    self.writer.color_matrix(
                        2,
                        second_matrix.0,
                        matrix_to_tiff_value(&second_matrix.1, 10_000).as_slice(),
                    );
                }
            }
        }
        ...
        self.ifd_mut()
            .add_tag(TiffCommonTag::ImageWidth, rawimage.width as u32);
        self.ifd_mut()
            .add_tag(TiffCommonTag::ImageLength, rawimage.height as u32);

        self.ifd_mut()
            .add_tag(DngTag::ActiveArea, rect_to_dng_area(&active_area));
        ...
        match cropmode {
            CropMode::ActiveArea => {
                ...
                self.ifd_mut().add_tag(
                    ...
                    [
                        (crop.p.x - active_area.p.x) as u16,
                        (crop.p.y - active_area.p.y) as u16,
                    ],
                );
                self.ifd_mut()
                    .add_tag(DngTag::DefaultCropSize, [crop.d.w as u16, crop.d.h as u16]);
            }
            CropMode::Best => {
        ...
        self.ifd_mut().add_tag(
            ...
            [
                Rational::new(
                    rawimage.camera.default_scale.0[0][0],
                    rawimage.camera.default_scale.0[0][1],
                ),
                Rational::new(
                    rawimage.camera.default_scale.0[1][0],
                    rawimage.camera.default_scale.0[1][1],
                ),
            ],
        );
        self.ifd_mut().add_tag(
            ...
            Rational::new(
                rawimage.camera.best_quality_scale.0[0],
                rawimage.camera.best_quality_scale.0[1],
            ),
        );

        // Whitelevel
        assert_eq!(
            rawimage.whitelevel.0.len(),
            rawimage.cpp,
            "Whitelevel sample count must match cpp"
        );
            ...
            .all(|x| *x <= (u16::MAX as u32))
        {
            ...
            self.ifd_mut()
                .add_tag(DngTag::WhiteLevel, &rawimage.whitelevel.0);
        }
        ...
        self.ifd_mut().add_tag(
            DngTag::BlackLevelRepeatDim,
            [blacklevel.height as u16, blacklevel.width as u16],
        );
        ...
        if blacklevel.levels.iter().all(|x| x.d == 1) {
            ...
            if payload.iter().all(|x| *x <= (u16::MAX as u32)) {
                ...
                self.ifd_mut().add_tag(
                    DngTag::BlackLevel,
                    &payload.into_iter().map(|x| x as u16).collect::<Vec<u16>>(),
                );
            } else {
                // Add as u32
            ...
            // Add as RATIONAL
            self.ifd_mut()
                .add_tag(DngTag::BlackLevel, blacklevel.levels.as_slice());
        }

        if !rawimage.blackareas.is_empty() {
            let data: Vec<u16> = rawimage
                .blackareas
                .iter()
                .flat_map(rect_to_dng_area)
                .collect();
            self.ifd_mut().add_tag(DngTag::MaskedAreas, &data);
        }

        self.ifd_mut()
            .add_tag(TiffCommonTag::SamplesPerPixel, rawimage.cpp as u16);
        ...
        match &rawimage.photometric {
            ...
            RawPhotometricInterpretation::Cfa(config) => {
                ...
                self.ifd_mut().add_tag(
                    ...
                    [cfa.width as u16, cfa.height as u16],
                );
                self.ifd_mut()
                    .add_tag(TiffCommonTag::CFAPattern, &cfa.flat_pattern()[..]);
                self.ifd_mut().add_tag(
                    TiffCommonTag::PhotometricInt,
                    PhotometricInterpretation::CFA,
                );
                self.ifd_mut()
                    .add_tag(DngTag::CFAPlaneColor, &config.colors);
        ...
        match compression {
            DngCompression::Uncompressed => {
                self.ifd_mut()
                    .add_tag(TiffCommonTag::Compression, CompressionMethod::None);
                dng_put_raw_uncompressed(self, &rawimage)?;
            }
            DngCompression::Lossless => {
                self.ifd_mut()
                    .add_tag(TiffCommonTag::Compression, CompressionMethod::ModernJPEG);
    ...
    pub fn preview(&mut self, img: &DynamicImage, quality: f32) -> Result<()> {
        ...
        self.ifd_mut()
            .add_tag(TiffCommonTag::ImageWidth, Value::long(preview_img.width()));
        self.ifd_mut().add_tag(
            TiffCommonTag::ImageLength,
            Value::long(preview_img.height()),
        );
        self.ifd_mut()
            .add_tag(TiffCommonTag::Compression, CompressionMethod::ModernJPEG);
        self.ifd_mut()
            .add_tag(TiffCommonTag::BitsPerSample, [8_u16, 8, 8]);
        self.ifd_mut()
            .add_tag(TiffCommonTag::SampleFormat, [1_u16, 1, 1]);
        self.ifd_mut().add_tag(
            TiffCommonTag::PhotometricInt,
            PhotometricInterpretation::YCbCr,
        );
        self.ifd_mut().add_tag(
            TiffCommonTag::RowsPerStrip,
            Value::long(preview_img.height()),
        );
        self.ifd_mut()
            .add_tag(TiffCommonTag::SamplesPerPixel, 3_u16);
        self.ifd_mut()
            .add_tag(DngTag::PreviewColorSpace, PreviewColorSpace::SRgb); // ??
        ...
        let jpeg_encoder = JpegEncoder::new_with_quality(
            &mut self.writer.dng.writer,
            (quality * 100.0).max(100.0) as u8,
        );
        preview_img
            ...
            .map_err(|err| {
                io::Error::new(
                    io::ErrorKind::Other,
                    format!("Failed to write jpeg preview: {:?}", err),
                )
            })?;
        ...
        self.ifd_mut()
            .add_value(TiffCommonTag::StripOffsets, Value::Long(vec![offset]));
        self.ifd_mut()
            .add_tag(TiffCommonTag::StripByteCounts, Value::Long(vec![data_len]));
    ...
    B: Write + Seek,
{
    ...
    pub fn color_matrix(&mut self, slot: usize, illu: Illuminant, matrix: impl AsRef<[SRational]>) {
        match slot {
            1 => {
                self.root_ifd
                    .add_tag(DngTag::CalibrationIlluminant1, u16::from(illu));
                self.root_ifd.add_tag(DngTag::ColorMatrix1, matrix.as_ref());
            }
            2 => {
                self.root_ifd
                    .add_tag(DngTag::CalibrationIlluminant2, u16::from(illu));
    ...
    pub fn load_metadata(&mut self, metadata: &RawMetadata) -> Result<()> {
        ...
        transfer_entry(
            &mut self.root_ifd,
            DngTag::LensInfo,
            &metadata.exif.lens_spec,
        )?;

        if let Some(id) = &metadata.unique_image_id {
            self.root_ifd
                .add_tag(DngTag::RawDataUniqueID, id.to_le_bytes());
        }
        Ok(())
    }
    ...
    pub fn xpacket(&mut self, xpacket: impl AsRef<[u8]>) -> Result<()> {
        self.root_ifd
            .add_tag(ExifTag::ApplicationNotes, xpacket.as_ref());
    ...
    pub fn load_base_tags(&mut self, rawimage: &RawImage) -> Result<()> {
        self.root_ifd
            .add_tag(TiffCommonTag::Make, rawimage.clean_make.as_str());
        self.root_ifd
            .add_tag(TiffCommonTag::Model, rawimage.clean_model.as_str());
        let uq_model = format!("{} {}", rawimage.clean_make, rawimage.clean_model);
        self.root_ifd
            .add_tag(DngTag::UniqueCameraModel, uq_model.as_str());
        Ok(())
    }
    ...
    pub fn close(mut self) -> Result<()> {
        if !self.exif_ifd.is_empty() {
            ...
            self.root_ifd
                .add_tag(TiffCommonTag::ExifIFDPointer, exif_ifd_offset);
        }
    ...
    pub fn original_file(
        ...
        self.root_ifd
            .add_tag_undefined(DngTag::OriginalRawFileData, buf.into_inner());
        self.root_ifd
            .add_tag(DngTag::OriginalRawFileName, filename.as_ref());
    ...
    pub fn subframe(&'_ mut self, id: u32) -> SubFrameWriter<'_, B> {
        SubFrameWriter::new(self, id, false)
    }

    pub fn subframe_on_root(&'_ mut self, id: u32) -> SubFrameWriter<'_, B> {
    ...
    pub fn thumbnail(&mut self, img: &DynamicImage) -> Result<()> {
        ...
        self.root_ifd
            .add_tag(TiffCommonTag::ImageWidth, thumb_img.width() as u32);
        self.root_ifd
            .add_tag(TiffCommonTag::ImageLength, thumb_img.height() as u32);
        self.root_ifd
            .add_tag(TiffCommonTag::Compression, CompressionMethod::None);
        self.root_ifd
            .add_tag(TiffCommonTag::BitsPerSample, [8_u16, 8, 8]);
        ...
        self.root_ifd
            .add_tag(TiffCommonTag::StripByteCounts, thumb_img.len() as u32);
        self.root_ifd
            .add_tag(TiffCommonTag::RowsPerStrip, thumb_img.height() as u32);
...
/// RGRG
/// GBGB
fn dng_put_raw_ljpeg<W>(
    subframe: &mut SubFrameWriter<W>,
    rawimage: &RawImage,
    predictor: u8,
) -> Result<()>
where
    W: Seek + Write,
{
    ...
    let lj92_data = match rawimage.data {
        RawImageData::Integer(ref data) => {
            ...
            let tiled_data: Vec<Vec<u16>> = ImageTiler::new(
                data,
                rawimage.width,
                rawimage.height,
                rawimage.cpp,
            ...
            let (j_width, j_height, components, realign) = match &rawimage.photometric {
                ...
                RawPhotometricInterpretation::Cfa(config) => {
                    assert_eq!(rawimage.cpp, 1);
                    let realign = if (4..=7).contains(&predictor)
                        && config.cfa.width == 2
                        && config.cfa.height == 2
                    {
                        2
    ...
    subframe.ifd_mut().add_tag(
        TiffCommonTag::BitsPerSample,
        &vec![rawimage.bps as u16; rawimage.cpp],
    );
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::SampleFormat, &vec![1_u16; rawimage.cpp]);
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::TileOffsets, &tile_offsets);
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::TileByteCounts, &tile_sizes);
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::TileWidth, tile_w as u16);
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::TileLength, tile_h as u16);
    ...
    W: Write + Seek,
{
    ...
    let rows_per_strip = if rawimage.height > 1000 {
        256
    } else {
        rawimage.height
    };
    ...
    match rawimage.data {
        RawImageData::Integer(ref data) => {
            ...
            subframe
                .ifd_mut()
                .add_tag(TiffCommonTag::SampleFormat, &vec![1_u16; rawimage.cpp]); // Unsigned Integer
            subframe
                .ifd_mut()
                .add_tag(TiffCommonTag::BitsPerSample, &vec![16_u16; rawimage.cpp]);
        }
        RawImageData::Float(ref data) => {
            ...
            subframe
                .ifd_mut()
                .add_tag(TiffCommonTag::SampleFormat, &vec![3_u16; rawimage.cpp]); // Unsigned Integer// IEEE Float
            subframe
                .ifd_mut()
                .add_tag(TiffCommonTag::BitsPerSample, &vec![32_u16; rawimage.cpp]);
        }
    };
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::StripOffsets, &strip_offsets);
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::StripByteCounts, &strip_sizes);
    subframe
        .ifd_mut()
        .add_tag(TiffCommonTag::RowsPerStrip, &strip_rows);
...
mod tests {
    ...
    fn build_empty_dng() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        dng.close()?;
        let expected_output = [
            73, 73, 42, 0, 36, 0, 0, 0, 1, 0, 0, 144, 7, 0, 4, 0, 0, 0, 48, 50, 50, 48, 0, 0, 0, 0,
            0, 0, 84, 101, 115, 116, 0, 0, 0, 0, 4, 0, 59, 1, 2, 0, 5, 0, 0, 0, 28, 0, 0, 0, 105,
            135, 4, 0, 1, 0, 0, 0, 8, 0, 0, 0, 18, 198, 1, 0, 4, 0, 0, 0, 1, 6, 0, 0, 19, 198, 1,
            0, 4, 0, 0, 0, 1, 4, 0, 0, 0, 0, 0, 0,
        ];
    ...
    fn convert_canon_cr3_to_dng() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        let mut rawdb = PathBuf::from(
            std::env::var("RAWLER_RAWDB")
                .expect("RAWLER_RAWDB variable must be set in order to run RAW test!"),
        );
        rawdb.push("cameras/Canon/EOS R6/raw_modes/Canon EOS R6_RAW_ISO_100_nocrop_nodual.CR3");

        let rawfile = RawSource::new(&rawdb)?;

        let original_thread = std::thread::spawn(|| {
            OriginalCompressed::compress(&mut BufReader::new(File::open(rawdb).unwrap()))
        });
        ...
        let full_image = decoder
            .full_image(&rawfile, &RawDecodeParams::default())?
            .unwrap();

...
impl RawSource {
    ...
    pub fn subview_padded(&'_ self, offset: u64, size: u64) -> std::io::Result<PaddedBuf<'_>> {
        if offset + size <= self.len() as u64 {
            if offset + size + 16 <= self.len() as u64 {
                self.subview(offset, size + 16)
                    .map(|buf| PaddedBuf::new_ref(buf, size as usize))
            } else {
    ...
    pub fn subview_until_eof_padded(&'_ self, offset: u64) -> std::io::Result<PaddedBuf<'_>> {
        if offset < self.len() as u64 {
            ...
            Err(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                format!(
                    "subview_until_eof_padded(): Offset {} is behind EOF",
                    offset
                ),
            ))
        }

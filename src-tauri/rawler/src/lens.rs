...
impl LensResolver {
    ...
    fn lens_matcher(&'_ self) -> LensMatcher<'_> {
        LensMatcher {
    ...
    pub fn resolve(&self) -> Option<&'static LensDescription> {
        ...
        if first_try.is_some() {
            ...
            if second_try.is_none() {
                log::warn!(
                    "No lens definition found in database, search parameters: {}. {}",
                    self,
                    crate::ISSUE_HINT
                );
                if std::env::var("RAWLER_FAIL_NO_LENS")
                    .ok()
                    .map(|val| val == "1")
                    .unwrap_or(false)
                {
    ...
    fn resolve_internal(&self) -> Option<&'static LensDescription> {
        ...
        if let Some(name) = self.lens_keyname.as_ref().filter(|s| !s.is_empty()) {
            if let Some(db_entry) = LENSES_DB
                .iter()
                .find(|entry| entry.identifiers.name == Some(name.into()))
            {
        ...
        if let Some(nikon_id) = &self.nikon_id {
            if let Some(db_entry) = LENSES_DB
                .iter()
                .find(|entry| entry.identifiers.nikon_id == Some(nikon_id.clone()))
            {
                return Some(db_entry);
            }
        }
        ...
        if let Some(olympus_id) = &self.olympus_id {
            if let Some(db_entry) = LENSES_DB
                .iter()
                .find(|entry| entry.identifiers.olympus_id == Some(olympus_id.clone()))
            {
                return Some(db_entry);
            }
        }
        ...
        let matches: Vec<&LensDescription> = LENSES_DB
            ...
            .filter(|entry| {
                self.mounts
                    .as_ref()
                    .is_none_or(|mounts| mounts.contains(&entry.mount))
            })
            .filter(|entry| {
                self.lens_id.as_ref().is_none_or(|id| {
                    entry
                        .identifiers
                        .id
                        .as_ref()
            ...
            .filter(|entry| {
                self.lens_make
                    .as_ref()
                    .is_none_or(|make| entry.lens_make == *make)
            })
            .filter(|entry| {
                self.lens_model
                    .as_ref()
                    .is_none_or(|model| entry.lens_model == *model)
            })
            .filter(|entry| {
                self.focal_len.as_ref().is_none_or(|focal| {
                    *focal >= entry.focal_range[0] && *focal <= entry.focal_range[1]
                })
            })
            .filter(|entry| {
                self.aperture.as_ref().is_none_or(|ap| {
                    *ap >= entry.aperture_range[0] || *ap <= entry.aperture_range[1]
                })
            })
            .collect();
        match matches.len() {
...
fn build_lens_database() -> Option<Vec<LensDescription>> {
    ...
    for lens in toml.get("lenses")?.as_array()? {
        ...
        lenses.push(LensDescription {
            ...
            lens_name: lens_name
                .unwrap_or(&format!("{} {}", lens_make, lens_model))
                .into(),

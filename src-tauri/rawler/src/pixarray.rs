    ...
    T: Copy + Default + Send,
{
    ...
    pub fn update_dimension(&mut self, dim: Dim2) {
        if self.width * self.height == dim.w * dim.h {
            ...
            panic!(
                "Can not change dimension: mismatch with old dimension: {:?} vs. {:?}",
                self.dim(),
                dim
            );
        }
    ...
    pub fn pixel_rows(&'_ self) -> std::slice::ChunksExact<'_, T> {
        debug_assert!(self.initialized);
        self.data.chunks_exact(self.width)
    }

    pub fn pixel_rows_mut(&'_ mut self) -> std::slice::ChunksExactMut<'_, T> {
        debug_assert!(self.initialized);
    ...
    {
        debug_assert!(self.initialized);
        self.pixel_rows_mut().enumerate().for_each(|(row, rowbuf)| {
            rowbuf
                .iter_mut()
                .enumerate()
                .for_each(|(col, v)| *v = op(*v, row, col))
        });
    }
    ...
    [T; N]: Default,
{
    ...
    pub fn pixel_rows(&'_ self) -> std::slice::ChunksExact<'_, [T; N]> {
        self.data.chunks_exact(self.width)
    }

    pub fn pixel_rows_mut(&'_ mut self) -> std::slice::ChunksExactMut<'_, [T; N]> {
        self.data.chunks_exact_mut(self.width)
    }
